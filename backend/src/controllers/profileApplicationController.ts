...
import { z } from "zod";
import { PROFILE_APPLICATION_ERROR_CODES } from "../constants/errors/profileApplication/codes";
...
export const getApplicationsListHandler = async (
  req: Request,
  res: Response,
) => {
...
export const getApplicationDetailHandler = async (
  req: Request,
  res: Response,
) => {
  ...
  if (isNaN(applicationId)) {
    throw new AppError("ID hồ sơ không hợp lệ", {
      statusCode: 400,
      code: PROFILE_APPLICATION_ERROR_CODES.PROFILE_APPLICATION_DETAIL_NOT_FOUND,
      details: {
        formErrors: [],
        fieldErrors: {
          id: [PROFILE_APPLICATION_FIELD_ERROR_MESSAGES.APPLICATION_ID_INVALID],
        },
      },
    });
  }
...
export const getApplicationCertificatesHandler = async (
  req: Request,
  res: Response,
) => {
  const idParam = Array.isArray(req.params.id)
